...
// returns true if the data contains at least two spaces
...
HTTPparser::status_t
HTTPparser::addBytes( std::deque<byte>& bytes )
{
   if ( _status != Incomplete ) return _status;


   // append the bytes to data.
	while( !bytes.empty() )
	{
   	 _data.append( 1, bytes.front() );
   	 bytes.pop_front();
   }
